...
IF NOT EXISTS (SELECT 1 FROM SubscriptionPlans WHERE Id = 1) INSERT INTO SubscriptionPlans (Id, PlanName, Description, MonthlyPrice, YearlyPrice, MaxUsers, MaxInvoicesPerMonth, MaxCompanies, HasEInvoice, HasEWayBill, HasGstReconciliation, HasGstApiIntegration, HasAdvancedReports, IsActive, CreatedDate) VALUES (1, 'Professional', 'Default migration plan for existing GST data.', 2499, 24999, 15, 1000, 1, 1, 1, 1, 0, 1, 1, SYSUTCDATETIME());
...
IF NOT EXISTS (SELECT 1 FROM Tenants WHERE Id = 1) INSERT INTO Tenants (Id, TenantName, BusinessName, GSTIN, PANNumber, ContactPersonName, Email, PhoneNumber, Address, State, City, Pincode, IsActive, IsTrial, SubscriptionPlanId, SubscriptionStartDate, SubscriptionEndDate, CreatedDate, CreatedBy, IsDeleted) VALUES (1, 'demo-gst-company', 'Demo GST Company', '24ABCDE1234F1Z5', 'ABCDE1234F', 'Demo Tenant Admin', 'info@demogstcompany.com', '9876543210', '101, Business Plaza, Ring Road', 'Gujarat', 'Surat', '395003', 1, 0, 1, SYSUTCDATETIME(), DATEADD(year, 1, SYSUTCDATETIME()), SYSUTCDATETIME(), 'Migration', 0);
SET IDENTITY_INSERT Tenants OFF;");
            ...
            {
                migrationBuilder.AddColumn<int>(name: "TenantId", table: table, type: "int", nullable: false, defaultValue: 1);
            ...
            migrationBuilder.Sql("UPDATE AspNetRoles SET IsPlatformRole = 1 WHERE NormalizedName IN ('SUPER ADMIN','SUPPORT ADMIN');");
